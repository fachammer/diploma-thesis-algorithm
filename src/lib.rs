...
pub fn main_worker() {
    ...
    let onmessage_handler: Closure<dyn Fn(_)> = Closure::wrap(Box::new(
        move |event: MessageEvent| {
            ...
            let SearchProof { disequality } = measure! {bincode::deserialize(&encoded_request).expect("deserialize should work") };
            let proof_result = measure! { search_proof(&disequality) };
